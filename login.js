...
form.addEventListener("submit", (event)=>{
    ...
    if(username.value === "" ){
        ...
        usernameErrors.push("-please enter your username <br>")
    } else if(String(username.value).length < 8 || regex.test(username.value) || String(username.value).length > 20){
        usernameErrorDisplay()
        usernameErrors.push("username invalid <br>")
    }else {
    ...
    if(password.value === ""){
        ...
        passwordErrors.push("-please enter your password <br>")
    }else if(String(password.value).toLocaleLowerCase === "password" || String(password.value) < 10 || String(password.value).length > 20){
        passwordErrorDisplay()
        passwordErrors.push("-password is invalid <br>")
    }else {
...
document.addEventListener("click", event =>{
    if(menuOpened &&  !menu.contains(event.target)){
        menu.style.visibility = "hidden"
        menuOpened = false
    }
})
